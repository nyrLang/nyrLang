...
static bool call(ObjClosure* closure, int argCount) {
	if (argCount != closure->function->arity) {
		runtimeError("Expected %d arguments, got %d instead", closure->function->arity, argCount);
		return false;
	}
...
static bool callValue(Value callee, int argCount) {
		...
		switch (OBJ_TYPE(callee)) {
			...
			case OBJ_CLASS: {
				...
				if (tableGet(&cls->methods, vm.initString, &initializer)) {
					...
					runtimeError("Exoected 0 arguments, got %d instead", argCount);
					return false;
				}
				return true;
			}
	...
	runtimeError("Can only call funtions and classes");
	return false;
}
