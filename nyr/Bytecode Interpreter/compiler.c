...
static void declareVariable() {
	...
	for (int i = current->localCount - 1; i >= 0; i--) {
		...
		if (identifiersEqual(name, &local->name)) {
			error("Already a variable withthis name in this scope");
		}
	}
...
static void function(FunctionType type) {
	...
	endScope(); // **Should** be taken care of by endCompiler. but just to be sure
...
static void functionDeclaration() {
	uint8_t global = parseVariable("Expected funtion name");
	markInitialized();
...
static void forStatement() {
	...
	if (!match(TOKEN_SEMICOLON)) {
		expression();
		consume(TOKEN_SEMICOLON, "Expected ';' after loop test");
	...
	if (!match(TOKEN_RPAREN)) {
		...
		expression();
		emitByte(OP_POP);
		consume(TOKEN_LPAREN, "Expected ')' after for clauses");
...
static void returnStatement() {
	...
	if (match(TOKEN_SEMICOLON)) {
		...
		if (current->type == TYPE_INITIALIZER) {
			error("Can't return from a class initializer");
		}
		expression();
...
ParseRule rules[] = {
	...
	[TOKEN_AMPERSAND_AMPERSAND] = {NULL,     and_,   PREC_NONE},
	[TOKEN_PIPE]                = {NULL,     NULL,   PREC_NONE},
	[TOKEN_PIPE_PIPE]           = {NULL,     or_,    PREC_NONE},
